...
| `tls-auth` / inline block | `OpenVpnTlsAuthKey` material slot | optional, fixed profile-owned slot; mutually exclusive with `tls-crypt` |
...
TUN mode, foreground/non-daemon execution, no shell or plugin environment,
the helper-owned management/credential descriptor, helper-owned runtime log
descriptor, and helper-selected bounded status/timeout verbosity. Their
values are implementation constants or separately reviewed bounded helper
settings, never strings copied from a profile.
...
pull-filters, and route/DNS scripts remain rejected until they receive their
