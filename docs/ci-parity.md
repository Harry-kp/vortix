...
| Boundary (`boundary.yml`) | `cargo xtask` `check-subprocess`, `check-platform-leak`, `check-protocol-leak`, `check-no-shell-regressions` |
...
Changes that touch only Markdown, `LICENSE` or the changelog skip Test, Lint, Boundary and
Integration through `paths-ignore`; only the release `plan` job runs. Any other change runs
everything.
