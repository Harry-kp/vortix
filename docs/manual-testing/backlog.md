...
| 89 | IPv6 row reads `Protected ✓` on WG full tunnel covering `::/0` | Host has IPv6 (`curl -6 -s ifconfig.co/ip` returns an address). Import `wg-full.conf` (AllowedIPs `0.0.0.0/0, ::/0`); `vortix up wg-full`; open TUI. Security Guard IPv6 row reads `Protected` with green `✓`. `vortix status --json \| jq .data.security.ipv6_leak` returns `false`. #227 acceptance. | TUI rendering — needs real v6 host |
| 90 | IPv6 row reads `Protected ✓` on WG IPv6-only tunnel (`::/0` no v4) | Import a WG profile with `AllowedIPs = ::/0` only (no `0.0.0.0/0`); connect. Same IPv6 row + JSON expectations as #89. This is the reporter's literal config in issue #227. | TUI rendering — exact #227 reproduction |
| 91 | IPv6 row reads `Leaking ✗` on WG split tunnel that doesn't cover v6 | Host has IPv6. Import `wg-split.conf` (AllowedIPs `10.8.0.0/24` only); `vortix up wg-split`; open TUI. Security Guard IPv6 row reads `Leaking` with red `✗`. JSON `ipv6_leak` returns `true`. Confirms the fix doesn't overcorrect. | TUI rendering — needs real v6 host |
| 92 | IPv6 row reads `Inactive ─` on WG split with no v6 traffic | Host has NO IPv6 (`curl -6` times out). `vortix up wg-split`; IPv6 row reads `Inactive` with `─`. JSON `ipv6_leak` returns `false`. | TUI rendering — needs v6-less host (or `ip -6 route del default` to simulate) |
| 93 | IPv6 row reads `Inactive ─` with no tunnel + no v6 traffic | Host has NO IPv6, no VPN connected. IPv6 row reads `Inactive` with `─`. (`Exposed` verdict; v6 row is one of several Defense rows.) | TUI rendering |
| 94 | IPv6 row reads `Leaking ✗` with no tunnel + v6 traffic | Host has IPv6, no VPN. IPv6 row reads `Leaking` with `✗`. JSON `ipv6_leak` returns `true`. Real leak — there's no tunnel to be inside of. | TUI rendering — needs real v6 host |
| 95 | IPv6 row reads `Protected ✓` on OpenVPN with server-pushed v6 default route | Use `ovpn-cert.ovpn` or any OVPN profile where the server pushes `route-ipv6 ::/0`. Connect; TUI IPv6 row reads `Protected ✓`. JSON `ipv6_leak` returns `false`. Confirms the registry-based logic works for OVPN, not just WG. | TUI rendering — needs OVPN server pushing v6 default route |
| 96 | IPv6 row reads `Leaking ✗` on OpenVPN v4-only with host v6 | Connect any OVPN profile where the server pushes only `redirect-gateway` (v4) and NOT a v6 default route. Host has IPv6. IPv6 row reads `Leaking ✗`. The OVPN test droplets in `scripts/test-infra.sh` (ovpn-cert/auth/totp) all match this — v4-only push. | TUI rendering — confirms OVPN tunnels are also subject to the leak check |
| 97 | IPv6 row reads `Protected ✓` under multi-tunnel where at least one covers `::/0` | Connect `wg-full` (covers `::/0`) AS primary, then `wg-split` (v4-only split) as secondary. Both Connected. IPv6 row reads `Protected ✓` — v6 traffic routes through the primary. JSON `ipv6_leak` returns `false`. | TUI rendering — multi-tunnel composition |
| 98 | IPv6 row reads `Leaking ✗` under multi-tunnel where NO tunnel covers `::/0` | Connect two v4-only split tunnels (e.g. two `wg-split`-shaped profiles with different `AllowedIPs`). Host has IPv6. IPv6 row reads `Leaking ✗` — no tunnel covers v6, so v6 traffic escapes the whole multi-tunnel topology. | TUI rendering — multi-tunnel composition |
| 99 | IPv6 status flips correctly across connect → disconnect cycles | Start with `wg-full` connected: `Protected ✓`. `vortix down wg-full` → row flips to `Inactive ─` or `Leaking ✗` depending on host v6. `vortix up wg-full` again → `Protected ✓` within ~1 telemetry tick. No stale-state hangover. | TUI rendering — state transitions over time |
