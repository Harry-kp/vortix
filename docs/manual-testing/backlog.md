...
| 67 | Security Guard panel fits cleanly at 80×24 with no compaction drops | Resize the terminal to exactly 80×24; connect a primary VPN; verify the Security Guard panel shows both section words (`Identity`, `Defense`), all rows (IP, DNS, Killswitch, Encryption, IPv6), and the `Updated …` footer with no rows dropped or truncated mid-word | TUI rendering — cell-budget regression check for the polish redesign |
