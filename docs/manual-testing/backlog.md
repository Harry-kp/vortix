...
| 101 | Security Guard back face (EICAS) — clean / leak / disconnected at 80×24 | Connect a WG profile that routes all traffic via the tunnel. Press `f` on Security Guard. Pass (clean): single `PASS N/N sockets via primary <iface>` verdict row, blank rows for the absent ribbon, `scope: primary <iface>` footer, `Esc back` nav-hint. No truncation at 80×24. Force a leak with `curl --interface en0 https://example.com` and wait ~3 s for the next SocketAudit poll. Pass (leak): `FAIL N sockets · 1 leak` verdict + a `● LEAK pid …` ribbon row. Disconnect entirely and press `f` again. Pass (disconnected): `???  Not connected` + `scope: partial — not connected`. | TUI rendering — flip-panel back face, requires live socket inventory |
