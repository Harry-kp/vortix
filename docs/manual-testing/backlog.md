...
| 92 | IPv6 row reads `v4-only ─` on WG split with no v6 traffic | Host has NO IPv6 (`curl -6` times out). `vortix up wg-split`; IPv6 row reads `v4-only` with `─`. JSON `ipv6_leak` returns `false`. | TUI rendering — needs v6-less host (or `ip -6 route del default` to simulate) |
| 93 | IPv6 row reads `v4-only ─` with no tunnel + no v6 traffic | Host has NO IPv6, no VPN connected. IPv6 row reads `v4-only` with `─`. (`Exposed` verdict; v6 row is one of several Defense rows.) | TUI rendering |
...
| 99 | IPv6 status flips correctly across connect → disconnect cycles | Start with `wg-full` connected: `Protected ✓`. `vortix down wg-full` → row flips to `v4-only ─` or `Leaking ✗` depending on host v6. `vortix up wg-full` again → `Protected ✓` within ~1 telemetry tick. No stale-state hangover. | TUI rendering — state transitions over time |
