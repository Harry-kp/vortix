...
| 89 | v6 sub-row hidden when host has NO IPv6 | Host has NO IPv6 (`curl -6 -s --max-time 5 https://ifconfig.co/ip` times out). Disconnect any VPN; open TUI. Neither Real IP nor Exit IP row shows a `↳ v6 …` sub-line. #227 acceptance: no v6 connectivity → no v6 row to mis-diagnose. | TUI rendering — needs v6-less host |
| 90 | v6 caches as `real_ipv6` while fully disconnected | Host has IPv6 (`curl -6 -s --max-time 5 https://ifconfig.co/ip` returns e.g. `2401:4900:1c61:23c4:986b:ba07:57bc:c881`). No VPN. Open TUI. Real IP row shows the v4 + a `↳ v6 …` sub-line carrying the SAME address `curl -6` reports. Exit IP row mirrors it (same v6, ✓ sigil). #227 acceptance: ground-truth match between probe and curl. | TUI rendering — needs real v6 host |
| 91 | v6 sub-row reads ✓ on WG full tunnel covering `::/0` (real masking) | Host has IPv6. With NO VPN, run `curl -6 -s ifconfig.co/ip` and record the address as `REAL_V6`. Connect `wg-full.conf` (AllowedIPs `0.0.0.0/0, ::/0`). Re-run `curl -6 -s ifconfig.co/ip` and confirm it returns a DIFFERENT v6 (the tunnel's exit v6). Open TUI: Real IP `↳ v6` sub-line reads `REAL_V6`; Exit IP `↳ v6` sub-line reads the new address with green `✓`. #227 acceptance — confirms the fix uses ground-truth IP comparison, not AllowedIPs introspection. | TUI rendering — needs real v6 + server that exits v6 |
| 92 | v6 sub-row reads ✗ when public_ipv6 matches real_ipv6 (real leak) | Host has IPv6. With NO VPN, record `REAL_V6` via `curl -6 ifconfig.co/ip`. Connect `wg-full.conf`. If `curl -6 ifconfig.co/ip` still returns `REAL_V6` (server NAT'd only v4 — common with split-stack VPN endpoints), Exit IP `↳ v6` sub-line reads `REAL_V6` with red `✗` + sub-line "v6 exposed — matches real IPv6". This is the reporter's actual scenario from #227. | TUI rendering — needs real v6 leak (v4-only-NAT VPN endpoint) |
| 93 | v6 sub-row reads ✗ on WG split tunnel that doesn't touch v6 | Host has IPv6. Record `REAL_V6` via `curl -6 ifconfig.co/ip`. Connect `wg-split.conf` (AllowedIPs `10.8.0.0/24` only — no `::/0`). Re-run `curl -6 ifconfig.co/ip` — should still return `REAL_V6` (split tunnel has no v6 effect). TUI Exit IP `↳ v6` sub-line reads `REAL_V6` with red `✗`. | TUI rendering — split-tunnel real leak |
| 94 | v6 sub-row reads ✓ on OpenVPN with server-pushed v6 default route | Use any OVPN profile where the server pushes `route-ipv6 ::/0` AND exits v6 (rare — most OVPN test servers are v4-only). Record `REAL_V6` pre-connect. Connect; `curl -6 ifconfig.co/ip` returns a different v6. TUI Exit IP `↳ v6` reads the new address with `✓`. Confirms the comparison-based logic works for OVPN, not just WG. | TUI rendering — needs OVPN exiting v6 |
| 95 | v6 sub-row reads ✗ on OpenVPN v4-only with host v6 | Host has IPv6. OVPN test droplets in `scripts/test-infra.sh` (ovpn-cert/auth/totp) all push only `redirect-gateway` (v4). Record `REAL_V6` pre-connect; connect; `curl -6 ifconfig.co/ip` still returns `REAL_V6`. TUI Exit IP `↳ v6` reads `REAL_V6` with red `✗`. | TUI rendering — confirms v4-only OVPN topology surfaces the v6 leak |
| 96 | v6 sub-row reads ✓ under multi-tunnel where at least one carries v6 | Host has IPv6. Record `REAL_V6`. Connect `wg-full` (covers and exits `::/0`) as primary, then `wg-split` (v4-only) as secondary. Both Connected. `curl -6 ifconfig.co/ip` returns a v6 differing from `REAL_V6`. TUI Exit IP `↳ v6` reads the new address with `✓`. | TUI rendering — multi-tunnel composition |
| 97 | v6 sub-row reads ✗ under multi-tunnel where NO tunnel carries v6 | Host has IPv6. Record `REAL_V6`. Connect two v4-only split tunnels with different `AllowedIPs`. `curl -6 ifconfig.co/ip` returns `REAL_V6`. TUI Exit IP `↳ v6` reads `REAL_V6` with red `✗`. | TUI rendering — multi-tunnel composition |
| 98 | v6 sub-row flips correctly across connect → disconnect cycles | Host has IPv6. Connect `wg-full` (assumes server exits v6). Exit IP `↳ v6` reads ✓. `vortix down wg-full` → public_ipv6 re-probes; sub-line reads the same address as Real IP `↳ v6` with `✓` (both now report the disconnected real). `vortix up wg-full` again → Exit IP `↳ v6` returns to the masked value within ~1 telemetry tick. No stale-state hangover. | TUI rendering — state transitions over time |
| 99 | v6 sub-row reads pending sigil before first probe completes | Cold start: `vortix` launched while disconnected on a v6 host. In the first ~5s before the IPv6 probe returns, Real IP `↳ v6` sub-row (if it renders at all) shows `detecting…` with the `─` not-applicable sigil. After the first probe completes the cached address replaces `detecting…`. | TUI rendering — initial-tick race |
