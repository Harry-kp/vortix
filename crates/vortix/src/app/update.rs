...
impl App {
    ...
    fn handle_disconnect_result(&mut self, profile: String, success: bool, error: Option<String>) {
        ...
        if !still_disconnecting {
            ...
            // disconnect via its `CompleteDisconnect` reconcile arm the
            // moment the kernel confirms the session is gone, which also
            // runs the pending-connect drain and kill-switch sync exactly
    ...
    fn handle_toggle_killswitch(&mut self) {
        ...
        // the daemon owns it — refuse instead of arming a state
        // nothing watches (see `sync_killswitch` / `attach_daemon`).
        if self.daemon_attached() {
            self.log("SEC: Kill switch unavailable in daemon mode until the daemon owns it");
            self.show_toast(
    ...
    fn handle_sync_system_state(&mut self, active: Vec<ActiveSession>) {
        ...
        for snap in &snapshots {
            ...
            // Decision half lives in the shared classifier so the daemon supervisor reconciles
    ...
    /// Land the result of a daemon-routed write. Completion is mostly a log line — the next snapshot poll
