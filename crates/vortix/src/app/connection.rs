...
impl App {
    ...
    ///. Marks the profile in-flight so polled
    ...
    pub(crate) fn sync_killswitch(&mut self) {
        ...
        // the daemon owns it — the scanner's drop-handler is
    ...
    pub(crate) fn force_disconnect(&mut self) {
        ...
        if let Some((name, protocol, config_path, cmd_tx)) = force_info {
            ...
            // Daemon-attached: escalate through the daemon.
            if self.daemon_attached() {
                self.spawn_daemon_execute(
                    &name,
    ...
    /// ). Drives the real per-protocol teardown via
    ...
    fn disconnect_specific(&mut self, profile_name: &str) {
        ...
        // Daemon-attached: the daemon owns the teardown. No local
        // tunnel thread; kill-switch transitions are parked in attached
        // mode (see `sync_killswitch`) until the daemon owns them.
        if self.daemon_attached() {
            self.spawn_daemon_execute(
                profile_name,
                crate::message::DaemonWriteAction::Disconnect,
    ...
    pub(crate) fn cancel_connect(&mut self, idx: usize) {
        ...
        // the same "cancel = disconnect" semantic the CLI gets.
        if self.daemon_attached() {
            self.spawn_daemon_execute(
                &name,
                crate::message::DaemonWriteAction::Disconnect,
...
/// here is detection, not validation — the `OpenVPN` binary still owns
/// parse errors). Returns an empty list when nothing parses.
...
mod u7_conflict_tests {
    //! App-side conflict-detection wiring.
    //!
