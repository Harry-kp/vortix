...
impl AppConfig {
    ...
    pub const fn connect_operation_timeout_secs(
        ...
        let protocol_gate = match protocol {
            crate::profile::ProtocolKind::WireGuard => self.wireguard_handshake_timeout_secs,
            crate::profile::ProtocolKind::OpenVpn => self.connect_timeout,
        };
...
pub(crate) fn persist_theme_choice(
    ...
    let directory =
        crate::config::owned_file::open_owned_directory(config_dir, false, owner.0, owner.1)
    ...
    let bytes = crate::config::owned_file::read_owned_user_entry(
        &directory,
        CONFIG_FILE,
        owner.0,
        MAX_CONFIG_BYTES,
...
fn classify_theme_write(
    ...
    use crate::config::owned_file::AtomicWriteError;
...
pub fn load_effective_config(config_dir: &Path) -> Result<AppConfig, String> {
    ...
    let legacy_engine = crate::config::EngineSettings {
...
fn validate_engine_settings(engine: &crate::config::EngineSettings) -> Result<(), String> {
...
fn with_engine_settings(mut config: AppConfig, engine: crate::config::EngineSettings) -> AppConfig {
...
mod tests {
    ...
    fn lifecycle_deadlines_keep_foreground_work_out_of_the_retry_budget() {
        ...
        assert_eq!(
            config.connect_operation_timeout_secs(crate::profile::ProtocolKind::WireGuard),
            22
        );
        assert_eq!(
            config.connect_operation_timeout_secs(crate::profile::ProtocolKind::OpenVpn),
            37
        );
        assert_eq!(config.disconnect_operation_timeout_secs(), 32);
        assert_eq!(
            config.reconnect_operation_timeout_secs(crate::profile::ProtocolKind::WireGuard),
            52
        );
        assert_eq!(
            config.reconnect_operation_timeout_secs(crate::profile::ProtocolKind::OpenVpn),
            67
        );
    }
    ...
    fn invalid_wireguard_engine_settings_fail_before_factory_construction() {
        let invalid_timeout = crate::config::EngineSettings {
    ...
    fn published_theme_is_kept_when_directory_sync_is_uncertain() {
        let outcome = classify_theme_write(Err(
            crate::config::owned_file::AtomicWriteError::PublishedButDirectoryUnsynced(
                crate::config::owned_file::FileError::Capacity,
            ),
        ))
        .unwrap();
