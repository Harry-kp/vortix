...
pub enum InputMode {
    ...
    /// ). Fired by Shift+`D` from the sidebar when more than one
    ...
    AuthPrompt {
        ...
        /// overlay-open so the field cannot inherit prior state). Only
        /// used when `static_challenge_prompt.is_some()`.
        otp: String,
