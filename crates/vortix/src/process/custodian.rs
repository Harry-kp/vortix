...
pub(crate) fn contains_protocol_pid(
    ...
    Ok(crate::process::real::process_is_nonleader_group_member(
        process_id,
        handshake.pid,
    )?)
}
...
fn process_group_absent(pid: u32) -> Result<bool, CustodianError> {
    Ok(!crate::process::real::process_group_has_live_members(pid)?)
}
...
pub fn maybe_run_hidden_entrypoint() -> Option<i32> {
    ...
    if arg.as_deref() == Some(std::ffi::OsStr::new(crate::process::real::GUARDIAN_ARG)) {
        return Some(crate::process::real::run_guardian_entrypoint().map_or(70, |()| 0));
    }
